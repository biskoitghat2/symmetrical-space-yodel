...
const INITIAL_STATE = {
    // Output Product
    productName: '',
    targetProductId: '', // If selecting existing product
    quantity: '1',
    sku: '',
    ...
    // Target Profit Margin for pricing suggestion
    profitMarginPercent: '20',
};

export const ProductionForm: React.FC<ProductionFormProps> = ({ windowId }) => {
    const [formState, setFormState, clearDraft] = useDraft(windowId, INITIAL_STATE);
    const closeWindow = useWindowStore((state) => state.closeWindow);
    const { addProduction, products } = useDataStore();
    ...
    // Filter possible output products (for restocking)
    const producedProducts = products.filter(p => p.category === 'تولیدی' || p.category === 'محصول نهایی');

    // Calculations
    const totals = useMemo(() => {
        const totalRaw = formState.rawMaterials.reduce((acc, item) =>
            new Decimal(acc).plus(new Decimal(item.quantity).times(item.unitPrice)).toNumber()
            , 0);

        // Calculate potential sell value of raw materials (Opportunity Cost)
        const totalRawSellValue = formState.rawMaterials.reduce((acc, item) => {
            const originalProduct = products.find(p => p.id === item.productId);
            const sellPrice = originalProduct ? originalProduct.sellPrice : new Decimal(item.unitPrice).times(1.2).toNumber();
            return new Decimal(acc).plus(new Decimal(item.quantity).times(sellPrice)).toNumber();
        }, 0);

        const totalExternal = formState.costs.filter(c => !c.isInternal).reduce((acc, c) => new Decimal(acc).plus(c.amount).toNumber(), 0);
        const totalInternal = formState.costs.filter(c => c.isInternal).reduce((acc, c) => new Decimal(acc).plus(c.amount).toNumber(), 0);

        const totalCost = new Decimal(totalRaw).plus(totalExternal).plus(totalInternal).toNumber();
        const qty = parseFloat(formState.quantity) || 1;

        const unitCost = new Decimal(totalCost).dividedBy(qty).toNumber();
        const realUnitCost = new Decimal(totalRaw).plus(totalExternal).dividedBy(qty).toNumber(); // Without internal profit/labor markup

        const margin = parseInt(formState.profitMarginPercent, 10) || 0;
        const suggestedSell = new Decimal(unitCost).plus(new Decimal(unitCost).times(margin).dividedBy(100)).toNumber();

        return { totalRaw, totalRawSellValue, totalExternal, totalInternal, totalCost, unitCost, realUnitCost, suggestedSell };
    }, [formState.rawMaterials, formState.costs, formState.quantity, formState.profitMarginPercent, products]);

    // Handlers
    const handleAddRawMaterial = (product: Product) => {
        const exists = formState.rawMaterials.find(i => i.productId === product.id);
        if (exists) {
            showToast('warning', 'این کالا قبلاً اضافه شده است');
            return;
        }
        ...
        const newItem: InvoiceItem = {
            ...
            quantity: 1,
            unitPrice: product.buyPrice, // Raw material cost is buy price (or average cost)
    ...
    const handleSelectOutputProduct = (pid: string) => {
        if (pid === 'NEW') {
            setFormState(prev => ({ ...prev, targetProductId: '', productName: '' }));
        } else {
            const p = products.find(prod => prod.id === pid);
            if (p) {
                setFormState(prev => ({ ...prev, targetProductId: p.id, productName: p.name, sku: p.sku || '' }));
            }
        }
    };
    ...
    const handleSubmit = (status: 'COMPLETED' | 'IN_PROGRESS') => {
        if (!formState.productName || !formState.quantity || formState.rawMaterials.length === 0) {
            showToast('error', 'لطفاً نام محصول، تعداد و مواد اولیه را مشخص کنید');
            return;
        }
        ...
        const production: Production = {
            ...
            targetProductId: formState.targetProductId || undefined,
            quantity: parseFloat(formState.quantity),
            sku: formState.sku,
            rawMaterials: formState.rawMaterials,
            costs: formState.costs,
            totalRawMaterialCost: totals.totalRaw,
            totalExternalCost: totals.totalExternal,
            totalInternalCost: totals.totalInternal,
            finalCostPrice: Math.round(totals.unitCost),
            suggestedSellPrice: Math.round(totals.suggestedSell),
            status: status,
        ...
        if (status === 'COMPLETED') {
            showToast('success', 'تولید محصول تکمیل و به انبار اضافه شد');
        } else {
            showToast('info', 'پروژه تولید ایجاد شد. مواد اولیه کسر گردید.');
        }
        clearDraft();
    ...
    return (
        <div className="flex h-full bg-gray-100 dark:bg-neutral-900 overflow-hidden relative">
            ...
            {/* Right: Inputs & Lists */}
            <div className="flex-1 flex flex-col bg-white dark:bg-surface border-l border-gray-200 dark:border-neutral-800 overflow-y-auto">
                <div className="p-6 space-y-6">
                    {/* ... (Same input sections as before) ... */}

                    {/* 1. Output Product Section */}
                    <div className="bg-blue-50 dark:bg-blue-900/10 p-4 border border-blue-100 dark:border-blue-900/30 rounded-lg">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">انتخاب محصول (یا جدید)</label>
                                <Select
                                    onChange={handleSelectOutputProduct}
                                    value={formState.targetProductId || 'NEW'}
                                    options={[
                                        { value: 'NEW', label: '-- محصول جدید --' },
                                        ...producedProducts.map(p => ({ value: p.id, label: p.name })),
                                    ]}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">نام محصول</label>
                                <input
                                    type="text"
                                    ...
                                    placeholder="نام کالای ساخته شده"
                                    disabled={!!formState.targetProductId}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">تعداد تولید</label>
                                <input
                                    type="number"
                                    min="0"
                                    step={['عدد', 'بسته', 'دستگاه', 'کارتن', 'شاخه', 'جفت'].includes(products.find(p => p.id === formState.targetProductId)?.unit || 'عدد') ? "1" : "any"}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">کد محصول (SKU)</label>
                                <div className="flex gap-1">
                                    <input
                                        type="text"
                                        value={formState.sku}
                                        onChange={e => setFormState(prev => ({ ...prev, sku: e.target.value }))}
                                        className="w-full p-2 bg-white dark:bg-neutral-800 border border-gray-300 dark:border-neutral-700 outline-none text-sm font-mono"
                                        disabled={!!formState.targetProductId}
                                    />
                                    <button
                                        type="button"
                                        onClick={generateSku}
                                        disabled={!!formState.targetProductId}
                                        className="bg-gray-100 dark:bg-neutral-800 border border-gray-300 dark:border-neutral-700 p-2 text-gray-500 hover:text-primary dark:hover:text-white transition-colors"
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="font-bold text-gray-700 dark:text-gray-300 text-sm">مواد اولیه مصرفی</h4>
                            <button
                            ...
                            <table className="w-full text-right text-xs">
                                <thead className="bg-gray-50 dark:bg-neutral-900 font-bold text-gray-500">
                                    <tr>
                                        ...
                                        <th className="p-2 w-20 text-center">موجودی</th>
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-neutral-800">
                                    {formState.rawMaterials.map(item => {
                                        ...
                                        return (
                                            <tr key={item.id} className={isInsufficient ? 'bg-red-50 dark:bg-red-900/10' : ''}>
                                                <td className="p-2">{item.productName}</td>
                                                <td className={`p-2 text-center font-mono font-bold ${isInsufficient ? 'text-red-600' : 'text-emerald-600'}`}>
                                                    {availableStock}
                                                </td>
                                                <td className="p-2">
                                                    <input
                                                        type="number"
                                                        min="0"
                                                        step={['عدد', 'بسته', 'دستگاه', 'کارتن', 'شاخه', 'جفت'].includes(currentProduct?.unit || 'عدد') ? "1" : "any"}
                                                        value={item.quantity}
                                                        onChange={(e) => handleUpdateRawMaterial(item.id, Number(e.target.value))}
                                                        className={`w-full text-center bg-gray-50 dark:bg-neutral-800 border ${isInsufficient ? 'border-red-500' : 'border-gray-200 dark:border-neutral-700'} rounded p-1 outline-none`}
                                                    />
                                                </td>
                                    ...
                                    {formState.rawMaterials.length === 0 && (
                                        <tr><td colSpan={6} className="text-center py-4 text-gray-400">مواد اولیه را اضافه کنید</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
                    ...
                    {/* 3. Production Costs */}
                    <div>
                        <h4 className="font-bold text-gray-700 dark:text-gray-300 text-sm mb-2">هزینه‌های سربار / ساخت</h4>
                        <div className="flex gap-2 mb-2">
                            <input
                                type="text"
                                placeholder="شرح هزینه (مثلا: حقوق کارگر)"
            ...
            {/* Left: Summary */}
            <div className="w-[350px] bg-gray-50 dark:bg-neutral-900 border-r border-gray-200 dark:border-neutral-800 flex flex-col shadow-xl z-20">
                ...
                <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                    <div className="space-y-2 text-sm">
                        ...
                        {/* Raw Material Opportunity Cost Display */}
                        <div className="flex justify-between text-xs text-amber-600 dark:text-amber-500 pb-2 border-b border-dashed border-gray-300 dark:border-neutral-700">
                            <span>ارزش فروش مواد اولیه:</span>
                            <span className="font-mono font-bold">{totals.totalRawSellValue.toLocaleString()}</span>
                        </div>
                    ...
                    <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded border border-emerald-100 dark:border-emerald-900/30">
                        <div className="text-xs text-emerald-700 dark:text-emerald-400 mb-1 font-bold">بهای تمام شده هر واحد</div>
                        ...
                        <div className="text-[10px] text-emerald-600 mt-1">
                            (خالص: {Math.round(totals.realUnitCost).toLocaleString()})
                        </div>
                    </div>

                    <div className="bg-white dark:bg-black p-4 rounded border border-gray-200 dark:border-neutral-800">
                        <label className="text-xs font-bold block mb-2 text-gray-500">حاشیه سود پیشنهادی (%)</label>
                        <input
                            type="number"
                ...
                <div className="p-4 bg-white dark:bg-black border-t border-gray-200 dark:border-neutral-800 space-y-2">
                    <button
                        onClick={() => handleSubmit('COMPLETED')}
                        className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded shadow-lg transition-transform active:scale-95 flex justify-center items-center gap-2"
                    >
                        <DollarSign size={18} />
                        تکمیل آنی و افزودن به انبار
                    </button>
                    <button
                        onClick={() => handleSubmit('IN_PROGRESS')}
                        className="w-full py-2 bg-white dark:bg-neutral-800 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 font-bold rounded shadow-sm transition-transform active:scale-95 flex justify-center items-center gap-2"
                    >
                        <PlayCircle size={18} />
                        شروع پروژه ساخت
                    </button>
